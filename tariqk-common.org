...
*** Fixing the a common OAuth problem
In order to resolve the issue for [[https://github.com/gcr/tumblesocks][tumblesocks]], we're going to include the following fix, based on [[https://github.com/gcr/tumblesocks#troubleshooting][this]]:
#+BEGIN_SRC emacs-lisp
  (setq oauth-nonce-function 'oauth-internal-make-nonce)
#+END_SRC

Still doesn't work, though. Am going through the devs using [[https://github.com/gcr/tumblesocks/issues/12][this issue ticket]].
** Interface
...
*** Changes to =org-mode=
**** Set up agenda files and capture files
There are two things that need to be done here: set up =org-capture= and =org-agenda= files.
***** Link to all personal =.org= files.
Personal =.org= files are to be kept synchronised with another service. 

What's been done, honestly is to set up =org-capture= to quickly capture tasks and activities for both work and personal contexts, which need to be kept separately. But this will work as a first pass.
#+BEGIN_SRC emacs-lisp
  (if (file-exists-p "~/org/")
      (progn
        (setq org-default-notes-file (concat org-directory "/scrapbook.org"))
        (add-to-list 'org-agenda-files (concat org-directory "/agendas"))
        (setq org-capture-templates
              '(("w" "Work" entry (file+datetree (concat org-directory "/agendas/@work.org"))
                 "* TODO %T %? \n  %i\  %a")
                ("p" "Personal" entry (file+datetree (concat org-directory "/agendas/personal.org"))
                 "* TODO %T %? \n %i \ %a")))
...
#+END_SRC
**** Changes to =org-reveal=
Since I've found new use to =org-reveal=, here are the multiple changes that I've incorporated into that package:
***** Add org-presentation mode with =ox-reveal=, using reveal.js served via CDN
So, rather than install =reveal.js= locally by default, I'll use the [[http://cdn.jsdelivr.net/reveal.js/3.0.0/][reveal.js copy available on jsDelivr]], which means that while I may end up being dependent on a network, I'll at least not have to install anything additional.
...
***** Customize the title slide for =org-reveal=
I've found that, rather than the default =org-reveal= title slide, which is ugly as heck, that this particular HTML document does pretty well:

#+NAME: org-reveal-title-template
#+BEGIN_SRC html
  <h1>%t</h1><h2>%a<br /><small>(%e)</small></h2><p>Use arrow keys to navigate. <br /><code>ESC</code> loads overview of slides.</p>
#+END_SRC

This is incorporated into the =org-reveal-title-slide=, as follows:

#+BEGIN_SRC emacs-lisp :noweb yes
  (when (require 'ox-reveal nil 'noerror)
    (setq org-reveal-title-slide "<<org-reveal-title-template>>"))
#+END_SRC

**** Make LaTeX fragments bigger in org-mode
When you use =C-u C-u C-c C-x C-l=, normally the fragment size is to small to read.

Initially it was based on [[http://tex.stackexchange.com/questions/78501/change-size-of-the-inline-image-for-latex-fragment-in-emacs-org-mode][this]] response, +but then I realized, after several attempts, and based on commentary [[https://lists.gnu.org/archive/html/emacs-orgmode/2013-09/msg00768.html][here]], that just use =customize-variable= and take the resulting value, what's the issue?+ LOL NEVERMIND THE FIRST LINK WORKS, I FORGOT THAT IT'S =#+BEGIN_SRC emacs-lisp=, NOT =#+BEGIN_SRC elisp=, I'M A DODO:
